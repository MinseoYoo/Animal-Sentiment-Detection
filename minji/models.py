...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, 4)  # 3채널 입력으로 수정
        ...
        self.fc3 = nn.Linear(1000, 96)  # 96키포인트로 수정
...
class NaimishNet(nn.Module):
    def __init__(
        self,
        image_size,
        output_size=96,  # 48개의 키포인트 * 2 (x, y 좌표)
        kernels=(5, 5, 5, 5),
...
class Resnet18Grayscale(nn.Module):
    def __init__(self):
        ...
        self.resnet18 = models.resnet18(pretrained=True)
        # 수정: 첫 번째 레이어를 그레이스케일 이미지에 맞게 수정
        self.resnet18.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.resnet18.fc = nn.Linear(self.resnet18.fc.in_features, 48*2)  # 48개의 키포인트 예측 (x, y) 좌표
